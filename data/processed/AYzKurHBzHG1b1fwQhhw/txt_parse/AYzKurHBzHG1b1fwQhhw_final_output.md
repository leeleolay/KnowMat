...
> [Figure 1 AI Description]: The figure displays a three-dimensional molecular dynamics simulation cell showing atomic arrangements of a 1/2[̄101] dislocation in a crystalline material. The periodic boundary conditions are represented by arrows along the X and Z directions at the cell boundaries. Atoms are color-coded by their local potential energy or coordination, revealing the core structure of the dislocation and surrounding elastic strain fields. The simulation cell dimensions are typically on the nanometer scale, with the dislocation line extending through the periodic structure.
...
> [Figure 2 AI Description]: The figure presents a plot comparing correlation lengths versus distance for different dislocation types in equiatomic high-entropy alloys. It shows data curves for screw, 30°, 60°, and edge dislocations, allowing direct comparison of their decorrelation behavior. The correlation length values typically decrease rapidly at short distances from the dislocation core before leveling off, with the specific decay rates and asymptotic values varying by dislocation character. This data is used to characterize how atomic displacements persist around different dislocation types in the random multicomponent alloy structure.
...
> [Figure 3 AI Description]: This figure presents three panels showing the temporal evolution of potential energy per atom during hybrid Monte Carlo/Molecular Dynamics simulations at annealing temperatures of 300 K, 700 K, and 1100 K. The x-axis tracks the cumulative number of attempted atomic swaps, while the y-axis displays the corresponding potential energy values. The plots reveal how energy decreases and equilibrates over simulation steps, with higher temperatures typically showing larger energy fluctuations before reaching convergence. The rate of energy minimization and the final equilibrated energy values differ significantly across the three temperature conditions, illustrating the temperature-dependent dynamics of the annealing process.
...
> [Figure 4 AI Description]: This figure displays the Warren-Cowley short-range order parameter (WC) plotted for various elemental pairs across three annealing temperatures (300 K, 700 K, and 1100 K). The data reveals how chemical ordering or clustering tendencies in the alloy system evolve with increasing thermal treatment, with negative WC values indicating clustering behavior and positive values indicating ordering tendencies. The comparison across temperatures typically shows a systematic reduction in the magnitude of WC parameters as temperature increases, reflecting the disordering effect of thermal energy.
...
> [Figure 5 AI Description]: The figure displays energy maps of a screw dislocation core in equiatomic FeNiCrCoCu HEA, showing significant spatial variations in both potential energy and intrinsic stacking fault energy across the slip plane at 0 K. The black lines marking partial dislocations reveal a non-planar, wavy dislocation structure with the enhanced view in panel (c) clearly demonstrating waviness in the dislocation core configuration, indicating complex core splitting behavior in this multi-principal element alloy.
...
> [Figure 6 AI Description]: This figure compares dislocation mobility between pure Ni and the FeNiCrCoCu high-entropy alloy, plotting velocity versus stress/temperature for both screw and edge characters. Such plots typically reveal power-law relationships with material-specific coefficients, where the HEA generally shows lower velocities or steeper stress dependence than pure Ni. The comparison between screw and edge dislocations illustrates the anisotropic core behavior typical of BCC alloys, where screw dislocations move more slowly due to their non-planar core structure.
...
> [Figure 7 AI Description]: The figure compares dislocation core waviness between pure Ni and a CrCoCu-containing HEA (FeNiCrCoCu) for both screw and edge dislocations across three temperatures. The HEA exhibits substantially higher waviness than pure Ni, particularly for edge dislocations where the scale had to be shifted (0.25–0.55) compared to the other panels (0–0.30) to accommodate larger values. Waviness, quantified as the standard deviation of atomic position variation along the dislocation core, was calculated by averaging over 300 ps molecular dynamics simulations, showing temperature-dependent behavior for both materials and dislocation types.
...
> [Figure 8 AI Description]: The figure plots screw dislocation velocity versus applied shear stress on log-log axes for pure Ni (blue data) and equiatomic FeNiCrCoCu HEA (red data) at 300 K. Multiple curve fits are shown for each material using different fitting methods, with data points and corresponding fit lines. Error bars indicate ±1 standard deviation in velocity measurements. The plot demonstrates that dislocation velocity increases with shear stress and that the HEA exhibits higher critical resolved shear stress and different velocity-stress slopes compared to homogeneous Ni, with the various fitting methods yielding slightly different parameter estimates for both materials.
...
> [Figure 9 AI Description]: The figure presents dislocation velocity versus Schmid shear stress measurements for screw dislocations (a) and edge dislocations (b) across five materials tested at 300 K. The data shows a positive correlation between applied stress and dislocation velocity, with the equiatomic FeNiCrCoCu HEA exhibiting lower velocities compared to pure Ni at equivalent stress levels. The Cu-rich HEA and pure Cu demonstrate distinctly higher dislocation velocities than the Ni-rich compositions, indicating that copper addition significantly enhances dislocation mobility in these alloys.
...
> [Figure 10 AI Description]: The figure displays dislocation mobility curves showing the relationship between shear stress and dislocation velocity for four dislocation characters (screw, 30°, 60°, and edge) in the equiatomic FeNiCrCoCu high-entropy alloy at 300 K. The curves indicate that screw dislocations exhibit the lowest mobility (slowest velocity at a given stress), while edge dislocations show the highest mobility. The mobility generally increases with increasing shear stress for all dislocation types, following a non-linear relationship typical of dislocation glide in concentrated solid solutions.
...
> [Figure 11 AI Description]: Mobility curves compare dislocation glide velocity as a function of applied stress for 30° and 60° dislocation orientations in an equiatomic HEA versus homogeneous Cu at 300 K. The plots show distinct velocity-stress relationships for positive and negative motion directions, revealing directional dependence (asymmetric mobility). The HEA typically exhibits different mobility characteristics than pure Cu, with potentially higher critical resolved shear stress and altered velocity regimes, reflecting the complex, compositionally disordered lattice environment of the high entropy alloy.
...
> [Figure 12 AI Description]: The figure compares the waviness (position variation) of screw and edge partial dislocations across different model compositions, showing that edge dislocations consistently exhibit higher waviness than screw dislocations across all compositions. The waviness values for edge dislocations are approximately 1.5-2 times greater than those for screw dislocations at each composition point. Both dislocation types show increasing waviness with certain composition variations, with edge dislocations displaying more pronounced compositional dependence than screw dislocations.
...
> [Figure 13 AI Description]: This figure displays transmission electron microscopy images comparing the waviness characteristics of leading and trailing partial dislocations in 30° mixed dislocations within an equiatomic high-entropy alloy. The images show that the 60° and screw dislocation character of the leading partial significantly influences the extent of dislocation waviness, with the screw component typically exhibiting more pronounced wavy configurations compared to the 60° character. Scale bars indicate the nanoscale resolution needed to resolve these partial dislocation features.
